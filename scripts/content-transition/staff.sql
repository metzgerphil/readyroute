...
BEGIN
 ...
 before_state=public.rr_staff_speed_snapshot('{"known_version":null}'::jsonb);
 SELECT coalesce(jsonb_agg(to_jsonb(x) ORDER BY to_jsonb(x)::text),'[]'::jsonb) INTO originals FROM rr_pilot_20260906.driver_help_knowledge_records x;
 FOR row_data IN SELECT value FROM jsonb_array_elements(p->'rows') LOOP
  ...
  IF EXISTS(SELECT 1 FROM rr_pilot_20260906.driver_help_knowledge_records x WHERE x.knowledge_id=row_data->>'knowledge_id' AND x.version>2) THEN RAISE EXCEPTION 'NEWER_SOURCE_EXISTS'; END IF;
 ...
 after_state=public.rr_staff_speed_snapshot('{"known_version":null}'::jsonb);
