...
function LiveTest({ staff, refresh, readOnly }) {
  ...
  return <section>
    <h2>Phone 2.0 testing</h2><p>Ask new questions and follow-ups using the phone answering engine. Completed tests and submitted reviews are saved for follow-through. Use fictional examples without customer names, addresses, or tracking numbers.</p>
...
export default function StaffAnswerLabPage() {
  ...
  return <div className="staff-answer-lab"><header><h1>Ready Route answer testing</h1><p>Test answers and send feedback directly to the Ready Route review queue. The phone application uses 2.0.</p></header><nav className="lab-tabs" aria-label="Answer testing mode">{[['v2','Phone 2.0'],['current','Legacy 1.0'],['review','Saved A/B comparison']].map(([v,label]) => <button aria-pressed={mode === v} key={v} onClick={() => setParams({ mode: v })}>{label}</button>)}</nav>
    {status && <aside className="lab-budget">2.0 pilot AI cap: {usd(status.budget.ceiling_nano)} · used {usd(status.budget.spent_nano)} · reserved {usd(status.budget.held_nano)}.{status.budget.stopped ? ' Live AI paused.' : ''} Saved A/B review is free. This cap covers 2.0 staff AI only; Legacy 1.0 uses its existing service. <button onClick={() => refresh().catch(e => setError(e.message))}>Refresh</button></aside>}
    {status && <p role="status">{status.alignment?.state === 'matched' ? 'Phone engine match checked. Staff tests use separate saved history and the staff AI allowance.' : status.alignment?.state === 'different' ? 'The phone engine has changed since this staff release. Alignment needs review; saved feedback remains available.' : 'The phone engine match could not be checked right now. Saved feedback remains available.'}</p>}
    {error && <p role="alert">{error} <button onClick={() => refresh().catch(e => setError(e.message))}>Retry connection</button></p>}
    <div hidden={mode !== 'current'}><p>Legacy comparison only. This log is saved in this browser tab. Use Phone 2.0 to submit shared reviews.</p><RraTestPage allowFeedback={false} apiBase="/staff/driver-help" /></div>
