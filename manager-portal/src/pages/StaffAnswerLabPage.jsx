...
export default function StaffAnswerLabPage() {
  ...
  return <div className="staff-answer-lab"><header><h1>Ready Route answer testing</h1><p>Test answers and send feedback directly to the Ready Route review queue. {status ? `Staff testing uses Ready Route ${version}.` : 'Sign in to load the current staff testing version.'}</p></header><nav className="lab-tabs" aria-label="Answer testing mode">{[['v2',status ? `Test ${version}` : 'Live testing'],['current','Legacy 1.0'],['review','Saved A/B comparison']].map(([v,label]) => <button aria-pressed={mode === v} key={v} onClick={() => setParams({ mode: v })}>{label}</button>)}</nav>
