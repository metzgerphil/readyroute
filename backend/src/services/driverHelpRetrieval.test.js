...
test('visible answer removes repeated steps and internal routing language', () => {
  const structure = buildAnswerStructure(record({
    ...
    prohibited_actions: [
      'Do not route the stated dog hazard into unrelated locker questions.',
...
test('recognizes an explicitly stated Evening service before asking a premium-service clarification', () => {
  assert.equal(
    questionSatisfiesClarificationRequirement(
      'Which premium service is shown?',
      'I am early for an evening delivery. Can I still deliver?'
    ),
    true
  );
});
