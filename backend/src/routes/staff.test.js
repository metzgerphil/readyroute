...
test('POST /staff/request-password-reset sends a reset link for active staff', async () => {
  ...
  try {
    ...
    assert.equal(resetUrl.origin, 'https://staff.readyroute.org');
...
test('POST /staff/invites creates an invite and sends the invite email', async () => {
  ...
  try {
    ...
    const inviteUrl = new URL(sentInvites[0].inviteUrl);
    assert.equal(inviteUrl.origin, 'https://staff.readyroute.org');
    assert.match(inviteUrl.pathname, /\/readyroute\/accept-invite$/);
  } finally {
    await server.close();
  }
});
