...
create table if not exists public.readyroute_staff_company_access_sessions (
  ...
  user_agent text,
  created_at timestamptz not null default now(),
  constraint readyroute_staff_company_access_status_check check (status in ('active', 'ended', 'expired')),
  constraint readyroute_staff_company_access_reason_check check (char_length(reason) between 10 and 1000)
);
...
create table if not exists public.readyroute_operating_cost_templates (
  ...
  constraint readyroute_operating_cost_templates_category_check check (
    category in ('ai_tools', 'vercel', 'google_cloud_run', 'supabase', 'email', 'maps', 'apple_developer', 'stripe_fees', 'domains', 'software', 'other')
  )
);
...
create table if not exists public.readyroute_operating_costs (
  ...
  notes text,
  receipt_url text,
  template_id uuid references public.readyroute_operating_cost_templates(id) on delete set null,
...
begin
  if not exists (
    ...
    where conname = 'readyroute_staff_company_access_support_ticket_fk'
  ) then
    alter table public.readyroute_staff_company_access_sessions
      add constraint readyroute_staff_company_access_support_ticket_fk
...
create table if not exists public.support_ticket_messages (
  ...
  email_provider_id text,
  created_at timestamptz not null default now(),
  constraint support_ticket_messages_author_type_check check (author_type in ('staff', 'requester', 'system')),
  constraint support_ticket_messages_body_check check (char_length(body) between 1 and 12000)
);
...
create table if not exists public.support_ticket_attachments (
  ...
  created_at timestamptz not null default now(),
  constraint support_ticket_attachments_uploader_type_check check (uploader_type in ('staff', 'requester')),
  constraint support_ticket_attachments_size_check check (size_bytes > 0 and size_bytes <= 8388608),
...
create index if not exists support_ticket_attachments_ticket_created_idx on public.support_ticket_attachments(ticket_id, created_at);
create index if not exists support_ticket_events_ticket_created_idx on public.support_ticket_events(ticket_id, created_at);
create index if not exists readyroute_staff_company_access_staff_idx on public.readyroute_staff_company_access_sessions(staff_user_id, status, created_at desc);
create index if not exists readyroute_staff_company_access_account_idx on public.readyroute_staff_company_access_sessions(account_id, created_at desc);
create index if not exists readyroute_operating_cost_templates_active_idx on public.readyroute_operating_cost_templates(is_active, vendor);
create unique index if not exists readyroute_operating_costs_template_period_uidx on public.readyroute_operating_costs(template_id, period_month) where template_id is not null;
