...
4. Create the Supabase platform schemas/functions required by any public trigger definitions. The automated disposable restore uses a no-op `supabase_functions.http_request()` stub so recovery validation cannot call production webhooks.
5. Apply `backup/database/schema.sql` with `psql --set ON_ERROR_STOP=1`.
6. Apply `backup/database/data.sql` with the same setting.
7. Recreate the three private buckets, keeping `public = false`.
8. Upload each object to the bucket and path recorded in `backup/storage/manifest.json`.
9. Verify object sizes and checksums with `node scripts/verify-supabase-storage-backup.mjs backup/storage`.
10. Configure non-production webhook secrets, point a non-production API revision at the restored project, and run the production smoke suite.
11. Record the restore date, backup ID, row counts, object count, and smoke result.
