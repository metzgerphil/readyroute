...
| MyGroundBiz | Authenticated in-app browser session | Authentication expired after the latest 2026-08-09 check | In progress | Sixty-two primary source/location records, 87 navigation destinations, 67 explicit destination-backlog rows, 77 Safety Topic Library listings (74 unique URLs), and 100 exact monthly news-archive links are inventoried. The On the Road landing page, pickup-coordination resource index, Customer Alerts redirect, and complete 905-line Recent Customer Alerts page have durable captures. |
...
- `SRC-GDRIVE-FILE-0003` — all 78 workbook scenarios structurally and adversarially reviewed; retained as `SECONDARY_REFERENCE`.
...
- `SRC-MGB-DOC-0008` — all five page identities and upper visible regions reviewed and checksum-preserved; cropped lower regions of pages 2-5 and original PDF bytes remain.
...
- `SRC-MGB-DOC-0011` — all seven Dog Bite Prevention pages visually reviewed from authenticated page-addressed renders; pages 3-6 support the animal-encounter safety record, while original PDF bytes remain a durable-recapture target.
...
- OP-130 is fully reviewed from MyGroundBiz. OP-132 is fully reviewed in the browser. OP-135 was opened and page 1 of 5 was visually reviewed, but the file was not archived and pages 2-5 remain. OP-324, OP-321, current OP-207/OP-207Res, HZ-035, SF-920P, relay check-in/out, and linked vehicle-security standards remain open.
...
- The MyGroundBiz session was restored and verified on 2026-08-09. The complete rendered Recent Customer Alerts page and link inventory were captured while authenticated. A later restored session allowed page-addressed review of all seven Dog Bite Prevention pages; seven 1280×720 rendered screenshots and a hash manifest are retained, but the viewer download did not yield original PDF bytes. OP-135 remains the leading partial-source target: its native viewer has exposed page 1 but not a durable original or complete later-page review. The current/mainstream overlay in `reports/mainstream_driver_priority_scope.md` prioritizes OP-135, pickup procedures/reason codes, and controlling safety/forms. Queue orders 284–289 are the deferred 2017 FCC videos.

## Knowledge extracted

- An evidence-discovered taxonomy currently contains 56 nodes and 26 cross-procedure relationships.
- Its 56-row readiness projection identifies 19 all-verified/durable nodes, 26 mixed-status nodes, 10 populated nodes with no verified record, and the source-gated empty Relay branch; no verified-only branch currently has a capture gap.
...
- The authoritative JSONL layer currently contains 138 records: 83 VERIFIED, two CONFLICT, thirty-two HUMAN_REVIEW_REQUIRED, and twenty-one POTENTIALLY_OUTDATED.
...
- A complete-for-current-layer procedural decision map and 185 driver-language validation cases now exercise shorthand, misspellings, incomplete phrases, incorrect terminology, ambiguity, source conflicts, safety priority, rare exceptions, forms, evidence custody, and multi-record retrieval. Validator floors preserve at least ten explicit misspelling, incomplete-language, and terminology-error cases; all 33 multi-record cases have exact risk-family accountability, every one of the 138 operational records has at least one case, and each case declares its expected information sufficiency and response mode.
- Every formal case also has a validator-exact clarification strategy and stop rule. Eighteen direct cases ask no questions; the remaining 164 order only branch, safety, review, escalation, or version facts and stop when the necessary context is established.
- The retrieval-oracle index now contains 690 driver-language variants: 665 embedded record variants and 25 tagged supplemental test surfaces. A 138-row coverage ledger proves every record has a formal case plus both terse and context-rich wording.
...
- A 106-row source-to-knowledge coverage ledger reconciles every primary source to all 338 exact general-record mappings. Thirty-eight sources contribute general operational evidence; every zero-mapping source has an explicit inaccessible, unreviewed, container/landing/index, redirect, secondary, partial, durable-partial, or reviewed-context disposition. Customer-alert evidence is additionally reconciled through the separate 138-row alert source-to-knowledge ledger.
- A 3,158-row claim-to-evidence allocation ledger auto-allocates 1,820 single-fragment claims and records 859 reviewed multi-fragment allocations; 479 multi-fragment claims remain explicitly gated for human source review.
- A 138-row evidence-capture risk ledger separates operational knowledge status from evidence reproducibility. Every `VERIFIED` record has reviewable evidence; dog safety has complete hashed renders, while accident-scene response remains production-capture-gated because OP-130/OP-132 are transient and OP-135 has only partial hashed renders.
- Five partially reviewed MyGroundBiz primary sources, 41 accessible but unreviewed portal resources, and nine fully reviewed portal sources lacking original-source capture join the open Safety Topic, monthly news-archive, and navigation work in a deterministic 289-resource queue. Its state fields distinguish five `PARTIAL_REVIEW_OPEN`, 35 `UNREVIEWED_PRIMARY_OPEN`, six `UNREVIEWED_PRIMARY_CAPTURED_REVIEW_OPEN`, nine `REVIEWED_DURABLE_CAPTURE_OPEN`, and 234 `UNACQUIRED_OPEN` resources. The version-ambiguous pickup sheet is excluded from active knowledge and deprioritized in favor of current OP-321. Exact remaining alert-child obligations include Pure Country, Orvis, 2025/2026 ACT, the PetSmart SFS store list, and the 2023 Dick's exemption article; the locker article is fully reviewed and durably archived. Archive rows span January 2015 through August 2026 as exposed by the source and do not imply that missing months or child articles were reviewed.
...
- All 55 non-verified records now have an explicit resolution row identifying the required evidence or decision, owner class, source/acquisition dependencies, and publication gate. This prevents exception statuses from becoming indefinite prose-only queues.
