...
- `inventory/mygroundbiz_authenticated_acquisition_queue.csv`: deterministic 289-resource work queue covering five partially reviewed primary sources, 35 accessible unreviewed primary resources still needing acquisition, six durably captured videos needing complete audio-visual review, nine fully reviewed sources lacking original-source capture, and the remaining Safety Topic, news-archive, and navigation backlogs, including exact candidate-comparison, non-verified dependency, current-evidence, and derived taxonomy links plus authoritative work-state bases without treating titles as evidence.
