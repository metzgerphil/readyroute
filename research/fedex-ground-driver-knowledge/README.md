...
- `validation/candidate_gap_queue_reconciliation.jsonl`: exact one-row-per-gap routing to existing acquisition/backlog targets, new source or authority obligations, clarification-only handling, or context required before queueing.
...
- `validation/brightcove_video_capture_audit.md`: six-video acquisition, metadata, checksum, complete audio-visual review, and historical-context/no-canonical-extraction boundary.
