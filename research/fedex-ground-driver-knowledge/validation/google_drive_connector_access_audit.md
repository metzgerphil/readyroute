...
2. Listing the exact folder returns 37 direct file children and no folder child in the returned set.
3. The 37 children represent 33 unique byte objects; four `.ashx.pdf` uploads are byte-identical to earlier supplied copies of MGB-119, OP-119, Focus on Package Placement, and the Package Placement quick reference.
4. Every child was fetched through the authenticated connector as complete raw bytes and hashed independently. Decoded byte lengths equal provider sizes for all 37 files.
5. Every one of the 33 unique SHA-256 values exists in `inventory/source_checksums.sha256`. The two latest unique objects are the original Sideswipe Collisions and Summer Driving PDFs; both were archived, hashed, and reconciled to their earlier complete rendered reviews.

The durable 37-row provider snapshot is `inventory/google_drive_connector_snapshot_2026-08-10.csv`. It preserves the exact file ID, title, MIME type, size, SHA-256, provider creation/modification times, and folder identity for every returned child.

## Current disposition

- `SRC-GDRIVE-ROOT-0001` is `ACCESSIBLE` and its current 37-child connector snapshot is fully reconciled.
- `SRC-GDRIVE-BROWSER-ROOT-0001` remains the reviewed accessible folder snapshot.
- The original 17-file browser snapshot remains historically accurate for its capture time; the 20 later direct uploads are additional connector-visible children, not proof that the earlier browser inventory was incomplete at capture time.
- All 37 current children map to checksum-protected archive bytes. The two latest originals required source-identity reconciliation and closed six publication gates; no operational instruction changed.
