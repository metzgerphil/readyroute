...
The 123-row inventory now carries `metadata_recovery_status` and `metadata_recovery_basis` on every row. The validator requires exactly 26 `ORIGINAL_ROW_RETAINED` rows, 62 `RECONSTRUCTED_FROM_CONTROLLED_LEDGERS` rows, nine `NEW_AUTHORITATIVE_ROW_AFTER_RECOVERY` rows, 24 `ORIGINAL_BYTES_ACQUIRED_AND_FULLY_REVIEWED` rows, and two `OFFICIAL_PUBLIC_SOURCE_ACQUIRED` rows, with a nonblank basis on each. The original-byte category includes the owner-uploaded Sideswipe Collisions and Summer Driving PDFs, which were visually reconciled to their prior complete rendered reviews, plus the earlier authenticated portal downloads proven byte-identical to fully reviewed supplied copies. Portal OP-117 remains a distinct-byte source with complete rendered-page identity. This preserves the recovery boundary without concealing later evidence improvement.
