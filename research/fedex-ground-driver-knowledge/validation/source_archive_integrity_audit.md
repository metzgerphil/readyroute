...
Purpose: prove that every locally archived Google Drive source object is inventoried, checksum-protected, present on disk, unchanged since acquisition, and reconciled from the complete ZIP to its extracted source record. This audit covers the original durable Drive archive objects; `source_capture_coverage_audit.md` separately reconciles durable-capture status across all 121 primary sources.
...
- `inventory/source_checksums.sha256` contains 69 unique path/digest entries, including all 18 Google Drive archive objects, the complete FORGE 3.3, OP-130/132/135, Dog Bite Prevention, Equipment Terms, SRS/SRI, Vehicle Appearance, OP-117, pickup-reference originals, and all current durable MyGroundBiz captures.
- All 69 current SHA-256 digests match the manifest.
...
The broader 121-source capture ledger identifies seven fully reviewed and four partially reviewed MyGroundBiz sources that currently lack original source-byte archives. Two have complete hashed page renders and two have partial hashed page renders; the remaining reviewed sources are explicitly transient. Four byte-identical portal/Drive source pairs share controlled archive paths, so the 77 `LOCAL_ARCHIVE_HASHED` source rows resolve through the 69-entry checksum manifest. Their remaining original-byte gaps stay explicit and are not archive-integrity failures.
