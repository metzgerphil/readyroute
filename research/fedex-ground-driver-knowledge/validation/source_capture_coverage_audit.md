...
| `TRANSIENT_REVIEW_ARTIFACT_ONLY` | 5 | Source is fully reviewed, but only a review artifact and remote locator remain; no durable page/source capture exists |
...
All fully or partially reviewed primary sources have a source-specific review artifact. Seventy-seven of the 121 primary-source records resolve to durable, checksum-protected local source bytes; four byte-identical portal/Drive pairs share archive paths within the 69-entry checksum manifest. Eleven reviewed or partially reviewed MyGroundBiz sources lack original source-byte archives: two have complete hashed page renders, two have partial hashed page renders, five have only transient full-review artifacts, and two remain transient-partial. Six additional MyGroundBiz videos have durable bytes but remain `NOT_YET_REVIEWED`; their capture status permits local review but no operational extraction.
