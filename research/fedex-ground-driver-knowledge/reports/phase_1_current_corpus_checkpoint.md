...
This checkpoint is superseded for milestone status by `phase_1_mainstream_daily_driver_completion_2026-08-10.md`. The broader exhaustive-corpus backlog remains preserved, but it no longer blocks the owner-defined mainstream daily-driver milestone.

## Current corpus

- 123 primary source records: 87 fully reviewed, four partially reviewed, 25 not yet reviewed, and seven inaccessible. The exact supplied Drive connector folder is now accessible and its 37 current children reconcile to 33 unique registered archive hashes.
- 144 general operational knowledge records: 90 `VERIFIED`, 32 `HUMAN_REVIEW_REQUIRED`, 20 `POTENTIALLY_OUTDATED`, and two `CONFLICT`.
- 385 exact evidence objects and mapping rows across 228 unique knowledge/source pairs.
- 3,254 substantive claims: 1,734 unambiguous single-fragment claims and 1,520 human-allocated multi-fragment claims across 67 records; zero allocation rows remain pending.
...
- Canonical release: 90 `SOURCE_VERIFIED`, seven `READY_ROUTE_APPROVED`, 27 `PENDING_REVIEW`, and 20 `POTENTIALLY_OUTDATED`; all 97 status-eligible records are publication-ready and zero remain evidence-capture gated.
...
- `npm run knowledge:release`: release build and validation pass; 144 records, 123 sources, 192 cases, 97 publication-ready records, zero evidence-gated status-eligible records, seven active adjudications, and 34 change-log entries.
...
The remaining exhaustive-corpus sources stay in governed ledgers and retain their evidence/status boundaries. The project owner confirmed that specialist and non-daily-driver sources do not block the mainstream milestone; pending, outdated, or conflicting records still cannot provide definitive instructions.
...
The original source-bearing workspace at `/Volumes/USB322FD/readyroute` was compared and copied into the clone with ignore-existing semantics. The clone was already a strict archive superset: all 24 USB source files, 68 USB capture files, and 42 video-visual review files were present; the clone additionally contained later acquisitions and captures. All 73 currently registered archive checksums and the full transfer-defined portable/full-corpus validation stack pass after restoration and the latest acquisitions. See `validation/workspace_restoration_audit_2026-08-10.md`.

The owner subsequently uploaded the two exact original PDFs. `SRC-MGB-DOC-0038` and `SRC-MGB-DOC-0039` are now checksum-preserved, all fourteen original pages are visually reconciled to the prior reviews, and all six former publication gaps are closed without changing operational instructions. The connected folder now contains 37 direct files representing 33 unique hashes.

The subsequent restored-corpus adversarial pass reviewed all 144 current records, all 3,254 claims/allocations, 385 mappings, 192 formal cases, and 724 indexed variants. It found no substantive operational addition or source-alignment correction in the accessible reviewed mainstream layer. The only findings were stale QC-document scope/count statements, which were corrected without changing knowledge truth. This establishes diminishing yield for the current accessible layer only; it does not close the authenticated acquisition, referenced-source, conflict, or human-review queues.
