...
- Authenticated home page and 87 global-navigation destinations were mapped.
- Twenty-eight reviewed/partially reviewed navigation destinations have primary source rows; the other 59 are explicitly preserved one-for-one in the authenticated destination backlog.
...
- The 77 Safety Topic listings resolve to 74 unique URLs: Dog Bite Prevention is fully reviewed from its archived original PDF, and the other 73 unique documents are explicit, duplicate-aware acquisition backlog rows.
- A validator-enforced 272-resource queue covers four partially reviewed primary sources, 31 accessible but unreviewed primary resources, seven fully reviewed sources lacking original source bytes, and the remaining direct Safety Topic, exact monthly news-archive, and unreviewed navigation backlogs. It distinguishes four `PARTIAL_REVIEW_OPEN`, 25 `UNREVIEWED_PRIMARY_OPEN`, six `UNREVIEWED_PRIMARY_CAPTURED_REVIEW_OPEN`, seven `REVIEWED_DURABLE_CAPTURE_OPEN`, and 230 `UNACQUIRED_OPEN` rows. The remaining partial sources and mainstream unreviewed sources lead Wave 0. The six 2017 FCC videos and the version-ambiguous pickup sheet are deferred; the latter contributes no active code or procedure entries. Current OP-321 is the pickup-code target. Newly discovered linked forms, guides, and broken endpoints remain explicitly inventoried. Monthly archive pages span January 2015-August 2026 as exposed by the source; titles never substitute for content review.
- The focused current-news audit inspected the eight preserved 2026 monthly archive result captures and their 15 result entries. Several 2026 archive filters returned 2024/2025 entries, proving that the archive label/query cannot establish article currency. No result supplied a source-complete broadly applicable current driver procedure; administrative, promotional, seasonal, and historical links remain deferred, while the QVABV result reinforces the already prioritized current Equipment Terms family. See `validation/current_2026_news_archive_priority_audit.md`.
...
- OP-130, OP-132, OP-135, FORGE 3.3, Dog Bite Prevention, the current five-page P&D equipment terms, the January 2026 Vehicle Appearance FAQ, and the eight-page SRS/SRI FAQ are fully reviewed from archived originals. Dog Bite Prevention pages 3-6 support exact avoidance, approach, knockdown, and wound-response guidance. The older pickup references are archived as historical/corroborative context; newer OP-117/FORGE material controls.
- Four portal downloads are byte-identical to their reviewed Drive copies. The portal OP-117 differs in bytes but all 89 rendered pages match the reviewed Drive copy.
...
- 144 operational knowledge records: 90 VERIFIED, two CONFLICT, thirty-two HUMAN_REVIEW_REQUIRED, and twenty POTENTIALLY_OUTDATED.
...
- 383 source-to-knowledge mapping rows across 40 promoted evidence sources.
- 3,242 generated claim-provenance rows connect every current substantive rule/condition/step/documentation/prohibition/escalation/clarification/presentation claim to exact evidence fragments and supported scopes. The companion allocation ledger auto-allocates 1,774 single-fragment claims and preserves human-reviewed exact allocations for all 1,468 multi-fragment claims across 65 records, with zero pending allocation rows.
- 724 driver-question variants in the retrieval-oracle index: 699 embedded in operational records plus 25 tagged supplemental test paraphrases that close per-record short/extended surface gaps without changing source truth.
- 192 driver-language validation cases covering all 144 operational records, including explicit misspelling, incomplete-language, terminology-error, ambiguity, safety, conflict, version, and source-limit cases, plus 33 validator-accountable multi-record interactions with explicit sufficiency and response-mode expectations.
- A 192-row clarification-strategy index gives every formal case ordered candidate facts and a stop rule; safety-priority cases require immediate protective action before clarification, and all conflict/source-limit/version-limit cases disclose their gate before gathering context.
...
- 121 source-to-knowledge coverage rows reconcile every primary source to mapping counts, knowledge IDs, coverage disposition, evidence basis, and follow-up; 40 sources are mapped and all 81 zero-mapping sources are explicitly classified.
- 121 source-capture coverage rows independently reconcile review artifacts, durable archives, checksums, reproducibility limits, and required follow-up; hashed rendered pages preserve the complete dog, sideswipe, summer-driving, package-placement, and package-placement-quick-reference sources plus exact reviewed portions of Equipment Terms, the 2026 FAQs, five FORGE quick references, and FORGE 3.2. Complete original bytes now preserve OP-135 and FORGE 3.3.
- A 144-row evidence-capture risk ledger projects those source-level limits onto every knowledge record: 136 use only durable evidence, six use complete hashed rendered-page evidence, and two rely only on fully reviewed transient evidence. No active evidence comes from a partially reviewed source.
- Every `VERIFIED` record now has reviewable evidence. Dog safety and accident-scene response now use checksum-preserved original PDFs for their MyGroundBiz evidence; remaining capture gates concern other explicitly identified transient/render-only sources.
...
- The source-to-knowledge reconciliation audit makes all 121 source dispositions machine-checkable and exactly accounts for all 383 mappings; no zero-mapping source can silently pass without a basis and follow-up.
...
- The evidence-authority audit checks all 383 evidence objects, including the current Download Pickup List, MGB-119 reconciliation, current FORGE 3.3 allocations, complete OP-135 allocations, and earlier page-specific branches, and enforces reviewed authority, exact locators, partial-source disclosure, and exact evidence/mapping locator equality.
- The decision-logic audit covers all 144 current records and the validator requires exact record/map identifier coverage.
- The status-queue audit reconciles all 54 non-verified records against the conflict, human-review, and potentially-outdated indexes, clarifies the zero-record generic unresolved status, and enforces exact report/status validation.
- A 54-row non-verified resolution ledger assigns every exception record an exact evidence/decision requirement, authority class, publication gate, and source/acquisition dependency; 50 records link to one or more structured source gaps and 13 link directly to authenticated queue resources.
...
- The question-variant audit indexes all 699 embedded variants plus 25 supplemental test surfaces, preserves threshold-discovered collision pairs plus semantic refusal/damage/hand-sheet/pickup-sync/premium-reattempt/key-security/drop-box-barcode/dog-safety branches, and validator-enforces complete mapping, surface diversity, language-type signals, and explicit ambiguity tests.
- A 144-row record-language surface ledger proves that every record has at least four embedded variants, one formal case, one four-token-or-shorter surface, and one six-token-or-longer surface.
- The taxonomy audit reconciles all 144 records to valid root-to-child paths across 57 evidence-discovered nodes; 56 are record-backed and the missing relay procedure is the sole sourced coverage exception.
...
- The Safety Topic Library audit reconciled all 77 listings, three duplicate-URL groups, one fully reviewed rendered-page source, and 73 unique document obligations under validator-enforced backlog coverage; 71 remain distinct Wave 1 queue resources after exact primary-source reconciliation.
...
- The temporal/version audit verified date coverage and chronology across all 121 primary sources, 144 general records, 137 customer-alert records, and 383 general-record evidence objects; it status-gates time-sensitive evidence and preserves the malformed Safety Topic Library date `12/31/0` as an unresolved source display rather than inferring a year.
...
- The claim-level provenance audit normalized exact locator mismatches, maintains a deterministic 3,242-claim traceability index, and rejects stale claim text, evidence sets, source scopes, statuses, non-exact evidence/mapping locators, or allocation classes inconsistent with evidence cardinality. It requires every future multi-fragment claim to retain a reviewed exact fragment allocation before becoming trace-ready.
- The claim-to-evidence allocation pass created exact one-row-per-claim production gates: 1,774 single-fragment claims are auto-allocated and all 1,468 multi-fragment claims are human allocated across 65 records, leaving zero pending rows. Reviewed allocation batches repaired unsupported or overbroad scopes while preserving unresolved branches and source limits rather than inventing missing procedure.
- The non-verified-answer publication audit makes the complete-procedure limitation explicit at the start of all 32 human-review answers and validator-enforces source-limit, conflict, currency, verification-action, and escalation disclosures across all 54 non-verified records.
...
- The 2026-08-10 repeat adversarial/diminishing-yield pass reviewed the complete restored accessible mainstream layer—144 records, 3,242 claims/allocations, 383 mappings, 192 cases, and 724 variants—and found no substantive operational addition or correction. It corrected stale QC-report counts and an obsolete artificial procedure-length statement; full-corpus diminishing yield remains open until the unacquired and inaccessible source obligations are dispositioned.
- A validator-exact 27-row human-adjudication queue now subtracts the seven active approvals from the 34 research-layer conflict/human-review records. It preserves the current safe boundary, exact missing decision, owner class, source/backlog dependencies, answer-quality requirements, and publication gate for every canonical `PENDING_REVIEW` record; 20 mainstream P&D questions lead six compliance/qualification questions and one specialized linehaul question.
- A separate validator-exact 20-row version-confirmation queue covers every canonical `POTENTIALLY_OUTDATED` record. Nineteen rows route to the missing current FORGE/release source `REFSRC-022`; the pickup-service taxonomy row routes to `REFSRC-035`. Existing older source versions and locators remain preserved, and no row is promoted from a filename, date, recollection, or merely current-looking screen.
