...
| Keep status codes and pickup reasons context-safe | Achieved for current references | All 50 `DELIVERY_STATUS` and seven `PICKUP_REASON` records have reviewed-source traceability, symmetric collision warnings, validator-enforced operational-translation classifications, generated release snapshots, and a 17-case reference-language suite that preserves definition-versus-workflow, auto-applied, outside-scope, unknown-code, and selection-authority boundaries. | Acquire current OP-324/OP-321 and the 21 definition-only delivery workflows; deliberately reconcile any code-set change and expand reference-language cases without contaminating the candidate holdout. |
...
| Requirement | Status | Evidence | Remaining work |
|---|---|---|---|
| Generate diverse realistic questions from discovered source situations | Achieved for current records and references | 724 one-to-one indexed operational variants—699 embedded and 25 supplemental—plus 192 formal operational cases and 17 separately validated reference-language cases. A validator-exact 144-row ledger proves every operational record has terse and context-rich wording; the reference lane adds code comparison, namespace ambiguity, workflow-gap, and unknown-token signals from independently reviewed candidate prompts. | Continue canonical mapping of the 128 remaining candidate prompts while preserving all 32 holdout prompts; expand with new sources and execute the reference retrieval oracle only when implementation is separately authorized. |
